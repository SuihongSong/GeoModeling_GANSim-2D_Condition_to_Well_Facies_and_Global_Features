...
def setup_snapshot_image_grid(G, training_set,
    ...
    for idx in range(gw * gh):
        x = idx % gw; y = idx // gw
        while True:
            real, label, _, wellface = training_set.get_minibatch_np(1) # 
    ...
    return (gw, gh), reals, labels, wellfacies, latents
...
class TrainingSchedule:
    def __init__(
        ...
        lod_training_kimg       = 160,      # Thousands of real images to show before doubling the resolution.
        lod_transition_kimg     = 160,      # Thousands of real images to show when fading in new layers.
...
def train_progressive_gan(
    ...
    resume_run_id           = None, #'/scratch/users/suihong/ProGAN_MultiChannel_Reusults_ConditionedtoMultiConditions_TF/060-pgan-conditionalWellEnlargement-gf-2gpu/network-snapshot-004160.pkl',         
                                            # Run ID or network pkl to resume training from, None = start from scratch.
    resume_snapshot         = None,         # Snapshot index to resume training from, None = autodetect.
    resume_kimg             = 0,   #4160      # Assumed training progress at the beginning. Affects reporting and training schedule.
    resume_time             = 0): #2*3600+56*60+3         # seconds, Assumed wallclock time at the beginning. Affects reporting.
    ...
    with tf.name_scope('Inputs'):
        ...
        reals, labels, _, wellfacies  = training_set.get_minibatch_tf(minibatch_in)
    ...
    for gpu in range(config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            with tf.name_scope('G_loss'), tf.control_dependencies(lod_assign_ops):
                G_loss = tfutil.call_func_by_name(G=G_gpu, D=D_gpu, opt=G_opt, lod = lod_in, labels = labels_gpu, well_facies = wellfacies_gpu, minibatch_size=minibatch_split, **config.G_loss)
    ...
    grid_size, grid_reals, grid_labels, grid_wellfacies, grid_latents = setup_snapshot_image_grid(G, training_set, **config.grid)  
    sched = TrainingSchedule(total_kimg * 1000, training_set, **config.sched)    
    grid_wellfacies_process = np.concatenate(((grid_wellfacies > 0),  (grid_wellfacies - 1) * (grid_wellfacies > 0)), 1)    

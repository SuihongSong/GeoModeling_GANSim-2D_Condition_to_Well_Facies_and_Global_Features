...
# x: [minibatch, 2, resolution, resolution], 2: 0 for well locations (where 1 for well locations, 0 for no well locations); 1: facies code (where 0 for background facies, 1 for channel and levee facies).
...
def G_paper(
    ...
    wellfacies_in,                      # Forth input: wellfacies [minibatch, 2, resolution, resolution]: well locations and facies code. 
    ...
    **kwargs):                          # Ignore unrecognized keyword args.
    ...
    wellfacies_in.set_shape([None, 2, resolution, resolution])
    wellfacies_in = tf.cast(wellfacies_in, tf.float32)
    combo_in = tf.cast(tf.concat([latents_in, labels_in], axis=1), dtype)
    lod_in = tf.cast(tf.get_variable('lod', initializer=np.float32(0.0), trainable=False), dtype)

    # Building blocks.
    def block(x, wellfc, res): # res = 2..resolution_log2
        with tf.variable_scope('%dx%d' % (2**res, 2**res)):
            if res == 2: # 4x4
             ...
             #       wellfc_downscaled = wellfc_downscale2d_process(wellfc, factor=int(resolution/(2**res)))
             #       wellfc_downscaled_conv = apply_bias(conv2d(wellfc_downscaled, fmaps=wellfc_conv_channels, kernel=1, gain=1, use_wscale=use_wscale))
    ...
    if structure == 'linear':
        x = block(combo_in, wellfacies_in, 2)
        images_out = torgb(x, 2)
        for res in range(3, resolution_log2 + 1):
            lod = resolution_log2 - res
            x = block(x, wellfacies_in, res)
            img = torgb(x, res)
    ...
    if structure == 'recursive':
        def grow(x, wellfc, res, lod):
            y = block(x, wellfc, res)
            img = lambda: upscale2d(torgb(y, res), 2**lod)
            if res > 2: img = cset(img, (lod_in > lod), lambda: upscale2d(lerp(torgb(y, res), upscale2d(torgb(x, res - 1)), lod_in - lod), 2**lod))
            if lod > 0: img = cset(img, (lod_in < lod), lambda: grow(y, wellfc, res + 1, lod - 1))
            return img()
        images_out = grow(combo_in, wellfacies_in, 2, resolution_log2 - 2)

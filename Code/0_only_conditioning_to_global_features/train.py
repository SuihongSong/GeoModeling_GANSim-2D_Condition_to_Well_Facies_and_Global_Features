...
class TrainingSchedule:
    def __init__(
        ...
        lod_training_kimg       = 640,      # Thousands of real images to show before doubling the resolution.
        lod_transition_kimg     = 640,      # Thousands of real images to show when fading in new layers.
        ...
        tick_kimg_dict          = {4: 320, 8:320, 16:320, 32:320, 64:320}): # Resolution-specific overrides.
...
def train_progressive_gan(
    ...
    with tf.name_scope('Inputs'):
        ...
        reals, labels, _, _  = training_set.get_minibatch_tf(minibatch_in)
    ...
    for gpu in range(config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            with tf.name_scope('G_loss'), tf.control_dependencies(lod_assign_ops):
                G_loss = tfutil.call_func_by_name(G=G_gpu, D=D_gpu, opt=G_opt, lod = lod_in, training_set=training_set, labels=labels_gpu,  minibatch_size=minibatch_split, **config.G_loss)
    ...
    misc.save_image_grid(grid_reals/255, os.path.join(result_subdir, 'reals.png'), drange=[0, 1], grid_size=grid_size)
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            if cur_tick % image_snapshot_ticks == 0 or done:
                #grid_fakes = Gs.run(grid_latents, grid_labels, minibatch_size=sched.minibatch//config.num_gpus)
                grid_fakes = G.run(grid_latents, grid_labels, minibatch_size=sched.minibatch//config.num_gpus)
